...
"""def_lookup, do_define_form, and lambdas"""

(define (test a b) (- a b))
(test 10 5)
;expect 5

(define (testagain a b) (+ a b)))
(testagain 10 5)
;expect 15
...
"""Conditions, do_and, do_or, do_if, do_cond"""
(and)
;expect True
...
(if #f (display 'hello) (display 'okay))
;expect okayokay
  ...
  (cond ((atom? expr)
        ...
        ((let? expr)
         (let ((values (cadr expr))
               (body   (cddr expr)))
           (cons (cons 'lambda (cons (car (zip (analyze values))) (map analyze body)))
